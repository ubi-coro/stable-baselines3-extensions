"""
This file contains all stable-baselines3 callback tests as well as additional tests to 
check the extensions added by stable-baselines3-extensions.
"""
...
def test_eval_callback_logs_are_written_with_the_correct_timestep(tmp_path):
    ...
    eval_env = gym.make(env_id)
    eval_freq = 101
    eval_callback = EvalCallbackExt(eval_env, eval_freq=eval_freq, warn=False)
    model.learn(500, callback=eval_callback)
...
def test_eval_callback_load_data(tmp_path):
    # test whether the eval callback loads the data correctly if the log path exists
    train_env = DummyVecEnv([lambda: BitFlippingEnv(n_bits=3)] * 5)
    eval_env = DummyVecEnv([lambda: BitFlippingEnv(n_bits=3)])
    ...
    assert len(eval_callback.evaluations_timesteps) == 0
    assert len(eval_callback.evaluations_results) == 0
    assert len(eval_callback.evaluations_length) == 0
    assert len(eval_callback.evaluations_successes) == 0
    ...
    evaluations_timesteps = np.array(eval_callback.evaluations_timesteps.copy())
    evaluations_results = np.array(eval_callback.evaluations_results.copy())
    evaluations_length = np.array(eval_callback.evaluations_length.copy())
    evaluations_successes = np.array(eval_callback.evaluations_successes.copy())

    assert os.path.exists(os.path.join(tmp_path, "evaluations.npz"))
    ...
    num_items = evaluations_timesteps.shape[0]
    assert (np.array(eval_callback.evaluations_timesteps) == evaluations_timesteps).all()
    assert (np.array(eval_callback.evaluations_results) == evaluations_results).all()
    assert (np.array(eval_callback.evaluations_length) == evaluations_length).all()
    assert (np.array(eval_callback.evaluations_successes) == evaluations_successes).all()

    model.learn(500, callback=eval_callback)

    evaluations_timesteps_new = np.array(eval_callback.evaluations_timesteps)
    evaluations_results_new = np.array(eval_callback.evaluations_results)
    evaluations_length_new = np.array(eval_callback.evaluations_length)
    evaluations_successes_new = np.array(eval_callback.evaluations_successes)

    assert evaluations_timesteps_new.shape[0] > evaluations_timesteps.shape[0]
    assert evaluations_results_new.shape[0] > evaluations_results.shape[0]
    assert evaluations_length_new.shape[0] > evaluations_length.shape[0]
    assert evaluations_successes_new.shape[0] > evaluations_successes.shape[0]

    assert (evaluations_timesteps_new[:num_items] == evaluations_timesteps).all()
    assert (evaluations_results_new[:num_items] == evaluations_results).all()
    assert (evaluations_length_new[:num_items] == evaluations_length).all()
    assert (evaluations_successes_new[:num_items] == evaluations_successes).all()
