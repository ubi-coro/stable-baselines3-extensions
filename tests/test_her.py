"""
This file contains all stable-baselines3 tests for the HerReplayBuffer as well as 
additional tests to check the extensions added by stable-baselines3-extensions.
"""
...
def test_multiprocessing(model_class, image_obs_space):
    ...
    model = model_class("MultiInputPolicy", env, replay_buffer_class=HerReplayBufferExt, buffer_size=int(2e4), train_freq=4)
    model.learn(total_timesteps=150)
...
def test_truncate_last_trajectory(n_envs, recwarn, n_steps, handle_timeout_termination):
    ...
    replay_buffer = HerReplayBufferExt(
        buffer_size=int(1e4),
